				...
				if (client) {
					async.waterfall([
							...
							function processResults(err, res, callback) {
								...
								// Send one result per broadcast messsage
								
								// Message will be received by UI5 Application Logic
								// connection.attachMessage(function(oControlEvent) {
								// via jQuery.parseJSON(oControlEvent.getParameter("data"));
								// can be pushed into a oData model then
								
								for (var i = 0; i < res.length; i++) {
									wss.broadcast(res[i].PURCHASEORDERID + ": " + res[i].PURCHASEORDERITEM);
								}
								
								// Send all results in one broadcast messsage
								// wss.broadcast(res);
